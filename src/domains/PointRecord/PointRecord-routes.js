...
class PointRecordRoutes extends BaseRoutes {
	routes() {
		...
		this.router.get("/total-points/all", [
			authTokenMiddleware.authenticate,
			tryCatch(PointRecordController.employeeTotalPoints),
		]);

		this.router.get("/total-points/me", [
			authTokenMiddleware.authenticate,
			tryCatch(PointRecordController.myTotalPoints),
		]);
	}
}
